...
def save_solution_to_problem_exec(identifier, path):
    ...
    new_path = "results_code/" + "/".join(path.split('/')[1:len(path)]) + "/" + identifier
    ...
    if os.path.isfile(new_path):
        ...
        while True:
            expand += 1
            new_file_name = new_path.split(".cpp")[0] + str(expand) + ".cpp"
            if os.path.isfile(new_path):
                continue
            else:
    ...
    new_path = "results_code/" + "/".join(path.split('/')[1:len(path)]) + "/" + file_name
